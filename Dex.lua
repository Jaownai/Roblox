...
local function main()
  ...
  local function CreateClassIconMap()
    local defaultIcons = Lib.IconMap.newLinear("rbxasset://textures/ClassImages.png", 16, 16)

    local iconMap = {}

    function iconMap:GetLabel()
      local label = Instance.new("ImageLabel")
      self:SetupLabel(label)
      return label
    end

    function iconMap:SetupLabel(obj)
      defaultIcons:SetupLabel(obj)
    end

    function iconMap:Display(obj, index)
      if type(index) == "string" then
        local image = Icons[index]
        if image and image ~= "rbxassetid://" then
          obj.Image = image
      ...
      defaultIcons:Display(obj, tonumber(index) or 0)
      obj.ScaleType = Enum.ScaleType.Crop
    end

    function iconMap:DisplayByKey(obj, key)
      local image = Icons[key]
      if image and image ~= "rbxassetid://" then
        self:Display(obj, key)
      else
        defaultIcons:DisplayByKey(obj, key)
      end
    end

    function iconMap:SetDict(dict)
      defaultIcons:SetDict(dict)
    end
...
return search]==]
  ...
  Explorer.Init = function()
    Explorer.ClassIcons = CreateClassIconMap()
...
Main = (function()
  ...
  Main.CreateMainGui = function()
    ...
    Main.CreateApp({
      Name = "Rejoin", 
      Icon = "rbxassetid://76849082320169", 
      OnClick = function()
